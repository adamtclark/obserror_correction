...
deltaS_lvls = seq(-0.8, 0.8, by=0.1) #-10:10
...
plot_delta_function = function(simout, dobs, colu = 1, ylim=c(-1,1), doplot=TRUE) {
  simout$derror = dobs
  dS_real = round((simout$alpha_2_true-simout$alpha_1_true)/simout$alpha_1_true,1)
  dS_real[!(dS_real>(-0.9) & dS_real<0.9)] = NA # remove extreme turnover cases
  simout$dS_real = dS_real
  mod = try(lme(derror~-1+as.factor(dS_real),
            data = simout,
  ...
  dS_lst = sort(unique(dS_real))#sort(unique(simout$dS))
...
annotate_plot = function(xlm = c(-0.8, 0.8), ylm = c(-0.14, 0.14)) {
...
annotate_plot2 = function(xlm = c(-0.8, 0.8), ylm = c(-0.04, 0.14)) {
  text(xlm[2], ylm[2], "overestimate turnover", cex = 0.8, pos = 2)
  text(xlm[2], ylm[1], "underestimate turnover", cex = 0.8, pos = 2)
  
  #text(xlm[1], ylm[2], "overestimate turnover", cex = 0.8, pos = 4)
  #text(xlm[1], ylm[1], "underestimate turnover", cex = 0.8, pos = 4)
}
...
dobs_same = ((simout_rich$alpha_2_same-simout_rich$alpha_1_same)-dtrue)/simout_rich$alpha_1_true
plot_delta_function(simout = simout_rich,dobs = dobs_same, ylim = lm_alpha, colu = collst[3])
annotate_plot()

dobs_diff = ((simout_rich$alpha_2_diff-simout_rich$alpha_1_diff)-dtrue)/simout_rich$alpha_1_true
...
dobs_same = ((simout_shannon$alpha_2_same-simout_shannon$alpha_1_same)-dtrue)/simout_shannon$alpha_1_true
plot_delta_function(simout = simout_shannon,dobs = dobs_same, ylim = lm_alpha, colu = collst[3])
annotate_plot()

dobs_diff = ((simout_shannon$alpha_2_diff-simout_shannon$alpha_1_diff)-dtrue)/simout_shannon$alpha_1_true
...
dobs_same = ((simout_simpson$alpha_2_same-simout_simpson$alpha_1_same)-dtrue)/simout_simpson$alpha_1_true
plot_delta_function(simout = simout_simpson,dobs = dobs_same, ylim = lm_alpha, colu = collst[3])
annotate_plot()

dobs_diff = ((simout_simpson$alpha_2_diff-simout_simpson$alpha_1_diff)-dtrue)/simout_simpson$alpha_1_true
...
mtext(expression(paste("Simulated change in richness ", Delta, italic("S"), "/", italic(S))), side = 1, outer = TRUE, line = 0.6)

legend(-0.8, .16, c("Same Surveyor", "Different Surveyors"),
