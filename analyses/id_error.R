...
d$padat = apply(covdat, 1, function(x) sum(!is.na(x) & x==0))
d$Ntrials = apply(covdat, 1, function(x) sum(!is.na(x)))-1
ps = which(d$Ntrials>0)

mod_p0 <- brm(bf(padat | trials(Ntrials) ~ cov_mean + (1|SITE_CODE),
                 zi ~ (1|SITE_CODE)),
              data = d[ps,], family = zero_inflated_binomial(), cores = 2)
mod_p0 # R-hats all <= 1.01
#plot(mod_p0)
ntrial = 2
pred_p = predict(mod_p0, re_formula = ~ 0, newdata = data.frame(cov_mean = xsq, Ntrials = ntrial))
pred_p = pred_p/ntrial
#pred_p = t(apply(tmp, 2, function(x) quantile(x, probs = c(0.5, 0.025, 0.975))))

par(mar=c(4,4,2,2))
plot((d$cov_mean)[ps], (d$padat/d$Ntrials)[ps],
     pch = d$Ntrials[ps]-1,
     ylim = c(0, 1),
     xlim = c(0,1),
     cex = cex_level, col = adjustcolor(1, alpha_level),
     xlab = "Mean species-level cover", ylab = "Pr. identification error")
abline(h = 0, v = c(0,1), lty = 3)
#abline(a = 0, b = sqrt(2), lty = 3)

matlines(xsq, pred_p[,c("Estimate", "Q2.5", "Q97.5")], lty = c(1,2,2), col = 1)

sitelst = sort(unique(d$SITE_CODE[ps]))
collst2 = viridis::viridis(length(sitelst))
for(i in 1:length(sitelst)) {
  pred_p = predict(mod_p0, newdata = data.frame(cov_mean = xsq, SITE_CODE = sitelst[i], Ntrials = ntrial))
  pred_p = pred_p/ntrial
  matlines(xsq, pred_p[,c("Estimate", "Q2.5", "Q97.5")], lty = c(1,2,2), col = collst2[i])
}


bn = cut(d$cov_mean[ps], seq(0,1, length=11))
tmp = tapply((d$padat/d$Ntrials)[ps], bn, mean)

lines(seq(0,1,length=10), tmp, col=2)
